...
class Upload extends Model
{
	...
	{
		$this->database->query("INSERT INTO `" . DB_PREFIX . "attached_files` (`file_name`, `ext`, `file_type`, `file_type_id`, `user_id`, `date_of_joining`) VALUES (?, ?, ?, ?, ?, ?) ", array($data['file'], $data['ext'], $this->database->escape($data['type']), (int)$data['id'], (int)$data['user_id'], $data['datetime']));
	}

	public function deleteAttachments($data)
	{
		$this->database->query("DELETE FROM `" . DB_PREFIX . "attached_files` WHERE `file_name` = ? AND `file_type` = ? AND `file_type_id` = ?" , array($this->database->escape($data['name']), $data['type'], (int)$data['type_id']));
	}
