...
class Sender extends Model
{
	...
	{
		$query = $this->database->query("SELECT `id`, company AS name, 'contact' AS type FROM `" . DB_PREFIX . "contacts`");
		return $query->rows;
	}
	...
	public function getContactReceiver($data)
	{
		$query = $this->database->query("SELECT company AS name, email FROM `" . DB_PREFIX . "contacts` WHERE id IN (".implode(',',$data['user']).") ");
		return $query->rows;
	}
