...
class Types extends Model
{
	...
	{
		$query = $this->database->query("UPDATE `" . DB_PREFIX . "departments` SET `name` = ?, `status` = ? WHERE `id` = ? ", array($this->database->escape($data['name']), (int)$data['status'], (int)$data['id']));
		if ($query->num_rows > 0) {
			return true;
		} else {
			return false;
	...
	{
		$query = $this->database->query("INSERT INTO `" . DB_PREFIX . "departments` (`name`, `status`) VALUES (?, ?)", array($this->database->escape($data['name']), (int)$data['status']));
		if ($query->num_rows > 0) {
			return true;
		} else {
			return false;
