...
class EmailtemplateController extends Controller
{
	public function index()
	{
		...
		$this->response->setOutput($this->load->view('emailtemplate/template_form', $data));
	}
	...
	protected function validateField($data)
	{
		...
		if ($this->controller_commons->validateText($data['template'])) {
			$error_flag = true;
	...
	public function emailSettings()
	{
		...
		/* Set Page title and action */
		$data['page_title'] = $this->language->get('text_email_settings');
		$data['action'] = URL.DIR_ROUTE.'emailsettings';
		$data['token'] = hash('sha512', TOKEN . TOKEN_SALT);
		$data['page_active'] = 'emailsetting';

		$data['lang']= $this->language->all();
		/*Render Emailsetting list view*/
		$this->response->setOutput($this->load->view('emailtemplate/emailsetting', $data));
	}
	...
	public function emailSettingsAction()
	{
		...
		**/
		if (!isset($_POST['submit'])) {
			$this->url->redirect('emailsettings');
			exit();
		}
		$data = $this->url->post;
	...
	protected function validateSMTPField($data)
	{
		...
		if ($this->controller_commons->validateEmail($data['smtp']['fromemail'])) {
			$error_flag = true;
			$error['fromemail'] = 'From Email';
		}

		if ($this->controller_commons->validateText($data['smtp']['fromname'])) {
			$error_flag = true;
			$error['fromname'] = 'From Name';
		}

		if (!empty($data['smtp']['reply'])) {
			if ($this->controller_commons->validateEmail($data['smtp']['reply'])) {
				$error_flag = true;
		...
		if ($data['smtp']['status'] == "2") {
			if ($this->controller_commons->validateText($data['smtp']['host'])) {
				$error_flag = true;
				$error['host'] = 'SMTP Host';
			}

			if ($this->controller_commons->validateNumber($data['smtp']['port'])) {
				$error_flag = true;
				$error['port'] = 'SMTP Port';
			}

			if ($this->controller_commons->validateEmail($data['smtp']['username'])) {
				$error_flag = true;
				$error['username'] = 'SMTP Username';
			}

			if ($this->controller_commons->validateText($data['smtp']['password'])) {
				$error_flag = true;
