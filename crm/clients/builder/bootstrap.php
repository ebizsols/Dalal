...
//User Agent
$language = new Language($user_agent->getLangauge());
$repository->set('language', $language);


if (!$user_agent->isLogged()) {
	if ($query['route'] == '') {
		$query['route'] = 'login';
	} elseif ($query['route'] != 'login' && $query['route'] != 'register' && $query['route'] != 'forgotpassword' && $query['route'] != 'resetpassword') {
		$url->redirect('login');
	}
}
